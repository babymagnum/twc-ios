...
class PilihTujuanWisataVM: BaseViewModel {
    var listCluster = BehaviorRelay(value: [ClusterData]())
    var loadingCluster = BehaviorRelay(value: false)
    
    func getCluster(nc: UINavigationController?) {
        loadingCluster.accept(true)
        
        networking.cluster { (error, cluster, isExpired) in
            self.loadingCluster.accept(false)
            ...
            guard let _cluster = cluster else { return }
            
            if _cluster.success {
                self.listCluster.accept(_cluster.data ?? [ClusterData]())
            } else {
                if _cluster.code == self.constant.EXPIRED {
                    self.forceLogout(nc: nc)
                } else {
                    self.showAlertDialog(image: nil, message: _cluster.message, navigationController: nc)
                }
            }
        }
    }
